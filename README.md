...
\[
\hat{Y}_{t+1:t+H} = \mathrm{Reflector}(
  \mathrm{Generator}(X, N, C, B)
)
\]
...
```text
Investigator -> Baseline + Case Library -> Generator -> Reflector
```
...
performance. The Generator may adjust this reference using news and case
...
Create a `.env` file if you want to use the LLM Generator. Example only:
...
For non-LLM experiments, set `use_llm_generator: false` in
